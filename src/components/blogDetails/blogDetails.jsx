...
export function BlogDetails() {
  ...
  // الجزء الأول (قبل أول ##) هو المقدمة
  const intro = sections[0];
  // الأجزاء المتبقية (التي تبدأ بـ ##) سنقوم بمعالجتها
          ...
          <div className="col-lg-8 order-lg-1">
            <div className="post-content">
              <div className="post-content text-end" dir="rtl">
                ...
                {/* Loop على أقسام المقال وبنحوّل كل Section لبلوك UI مستقل */}
                {bodySections.map((section, index) => {
                  /**
                   * كل section بيكون String بالشكل ده:
                   * "عنوان القسم\n\nنص القسم"
                   * فبنقسمه على حسب السطور
                   */
                  const lines = section.trim().split("\n");

                  // أول سطر بنعتبره عنوان القسم
                  const sectionTitle = lines[0];

                  // باقي السطور هي محتوى القسم
                  const sectionText = lines.slice(1).join("\n");

                  // مجموعة أيقونات بسيطة للتنويع بين الأقسام
                  const icons = [
                    "fa-camera",
                    "fa-cog",
                    "fa-lightbulb",
                  ...
                  // اختيار أيقونة بناءً على ترتيب القسم
                  const currentIcon = icons[index % icons.length];

                  return (
                    <div key={index} className="mb-5">
                      {/* Header القسم: أيقونة + عنوان */}
